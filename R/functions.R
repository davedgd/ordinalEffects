...
BuildOrdinalPlotData <- function(theEffect, theModel, theData, unscale = TRUE, unscaleRounding = 3) {
  ...
  if (unscale) {
    ...
    for (term in theTerms) {
      ...
      if ( is.element("scaled:center", names(attributes(theData[, term]))) ) {
        ...
        plotDat[, term] <- round(eval(parse(text=paste("plotDat$", term, sep=""))) * attr(theData[, term], "scaled:scale") + attr(theData[, term], "scaled:center"), unscaleRounding)
      }
